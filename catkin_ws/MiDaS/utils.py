...
def write_depth(path, depth, bits=2):
    ...
    depth_L = (depth).astype(np.float64)
    print(depth_L.shape)
    depth_L = 0.00353127*depth_L + 0.00894166
    depth_L = 1/depth_L
    depth_min = depth_L.min()
    depth_max = depth_L.max()
    ...
    np.save(path + ".npy",depth_L.astype(np.float64))
    ...
    plt.imshow(depth_L,cmap="magma")
    plt.show()
    print(out.astype(np.float64).shape)
    # if bits == 1:
