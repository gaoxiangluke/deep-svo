...
def run(input_path, output_path, model_path, model_type="large", optimize=True):
    ...
    import csv
    fileindex = None
    csvfile = open("fileindex.csv", "r")
    fileindex = csv.reader(csvfile, delimiter=' ', quotechar='|')
    fileindexs = []
    for row in fileindex:
        fileindexs.append(row[0])
    for ind, img_name in enumerate(img_names):
        real_imgname = os.path.join(input_path, fileindexs[ind]+".png")
        print("  processing {} ({}/{})".format(real_imgname, ind + 1, num_images))

        # input

        img = utils.read_image(real_imgname)
        ...
        # output
        # modified image name
        i_name = "depth_"+str(count)
        count += 1
        filename = os.path.join(
            output_path, os.path.splitext(os.path.basename(i_name))[0]
        )
