    ...
    with torch.no_grad():
        for file in fileindexs:
            ...
            if args.pred_metric_depth:
                name_dest_npy = os.path.join(output_directory, "depth_{}.npy".format(idx))
                metric_depth = STEREO_SCALE_FACTOR * depth.cpu().numpy()
                print(metric_depth.shape)
                metric_depth = metric_depth.reshape(192,640)
                # Working version
                # im = pil.fromarray(metric_depth)
                # metric_depth = im.resize((376,1241),resample=pil.BOX)
                # metric_depth =  np.array(metric_depth).reshape(376,1241,1)
                # print(metric_depth.shape)

                #cv version
                #depth_cv = cv2.CreateMat(192, 640, cv2.CV_32FC3)
                disp_ = disp.cpu().numpy().astype(np.float32).reshape(192,640)
                metric_depth = metric_depth
                # metric_depth = metric_depth/255.0
                depth_cv = cv2.resize(metric_depth,(1241,376), interpolation = cv2.INTER_CUBIC)
                #depth_cv = cv2.flip(depth_cv,1)
                metric_depth = depth_cv
                ...
                plt.imshow(depth_cv,cmap="magma")
                plt.show()
                np.save(name_dest_npy, metric_depth.astype(np.float64))
                break
            else:
